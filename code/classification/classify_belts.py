...
def analysis_classification(edf, 
    ...
    print()
    print(f'---> Task done in {round((time.time() - timer_task1), 2)} sec!')
    print()
    ...
    fig.add_annotation(text=f'model: {model_name} - total time: {hours_conversion(dictionary["total_hours"])} - hours valid: {hours_conversion(dictionary["hours_valid"])} - total signal quality: {dictionary["total_signal_quality"]} - valid: {dictionary["is_valid"]}',
