...
def analysis_classification(edf, 
                            ...
                            stop_print=False):
    """
    Primary function for classifying a belt patient signal

    Parameters:

    -edf (--edf): edf file path containing time series data
    -model (--model): learning model architecture, either MLP, CNN, ResNet or LSTM
    -model_path (model_path): corresponds to the path of the model weights
    ...
    if model_path == '':
        saved_dir = os.path.dirname(os.path.abspath('util.py')) + f'/models/belts/{model_name}'
        ...
        info_path = os.path.normpath(model_path).split(os.sep)[:-2]
        info_path.append('info.txt')
        info_path = '/'.join(info_path)
    ...
    df_jawac = df_jawac.resample(str(downsampling_value) + 'S', on='times').median()
    ...
    if not stateful:
        ...
        if not sliding_window:
            X_test_seq = np.array([(df_jawac.loc[i:i + (size-1), :].data_chest).to_numpy() for i in range(0, len(df_jawac), size)], dtype=object)
        else:
            size = int(segmentation_value)   # in sec
            X_test_seq = np.array([(df_jawac.loc[i:i + (size-1), :].data_chest).to_numpy() for i in range(0, len(df_jawac), center_of_interest)], dtype=object)
        X_test_seq_temp = []
    ...
    df_jawac['label_chest'] = 1
    df_jawac['proba_chest'] = 0.5
    ...
    for i in range(len(classes)):
        if sliding_window:
            ...
            if not return_sequences:
                ...
                mask = df_jawac.index[i]
        if classes[i][0] == 0:
            df_jawac.loc[mask, ['label_chest']] = 0
        df_jawac.loc[mask, ['proba_chest']] = classes[i][1]
    ...
    if not stateful:
        size = int((1 / downsampling_value) * segmentation_value)
        if not sliding_window:
            X_test_seq = np.array([(df_jawac.loc[i:i + (size-1), :].data_abd).to_numpy() for i in range(0, len(df_jawac), size)], dtype=object)
        else:
            size = int(segmentation_value)   # in sec
            X_test_seq = np.array([(df_jawac.loc[i:i + (size-1), :].data_abd).to_numpy() for i in range(0, len(df_jawac), center_of_interest)], dtype=object)
        X_test_seq_temp = []
        for arr in X_test_seq:
            # if not return_sequences:
            #     arr = np.append(arr, pd.Series([np.var(arr)*1000]))
    ...
    df_jawac['label_abd'] = 1
    df_jawac['proba_abd'] = 0.5
    ...
    for i in range(len(classes)):
        if sliding_window:
            ...
            if not return_sequences:
                ...
                mask = df_jawac.index[i]
        if classes[i][0] == 0:
            df_jawac.loc[mask, ['label_abd']] = 0
        df_jawac.loc[mask, ['proba_abd']] = classes[i][1]
    ...
    df_label = pd.DataFrame(columns=['start', 'end', 'label', 'proba', 'signal_num', 'data_num'])
    saved_label_chest = df_jawac.iloc[0].label_chest
    saved_start_chest = df_jawac.iloc[0].times
    saved_label_abd = df_jawac.iloc[0].label_abd
    saved_start_abd = df_jawac.iloc[0].times
    proba_chest = []
    proba_abd = []
    for idx, row in df_jawac.iterrows():
        if len(row) > 0:
            proba_chest.append(row.proba_chest)
            if row.label_chest != saved_label_chest:
                temp = pd.DataFrame(data = [[saved_start_chest, row.times, saved_label_chest, np.mean(proba_chest), 0, len(proba_chest)]], columns=['start', 'end', 'label', 'proba', 'signal_num', 'data_num'])
                df_label = pd.concat([df_label, temp], ignore_index=True)
                saved_label_chest = row.label_chest
                saved_start_chest = row.times
                proba_chest = []
            proba_abd.append(row.proba_abd)
            if row.label_abd != saved_label_abd:
                temp = pd.DataFrame(data = [[saved_start_abd, row.times, saved_label_abd, np.mean(proba_abd), 1, len(proba_abd)]], columns=['start', 'end', 'label', 'proba', 'signal_num', 'data_num'])
                df_label = pd.concat([df_label, temp], ignore_index=True)
                saved_label_abd = row.label_abd
                saved_start_abd = row.times
                proba_abd = []
    if len(proba_chest) > 0:
        temp = pd.DataFrame(data = [[saved_start_chest, df_jawac.times.tolist()[-1], saved_label_chest, np.mean(proba_chest), 0, len(proba_chest)]], columns=['start', 'end', 'label', 'proba', 'signal_num', 'data_num'])
        df_label = pd.concat([df_label, temp], ignore_index=True)
    if len(proba_abd) > 0:
        temp = pd.DataFrame(data = [[saved_start_abd, df_jawac.times.tolist()[-1], saved_label_abd, np.mean(proba_abd), 1, len(proba_abd)]], columns=['start', 'end', 'label', 'proba', 'signal_num', 'data_num'])
        df_label = pd.concat([df_label, temp], ignore_index=True)
    ...
    # print(f'invalid count: {invalid_count} --> {round((invalid_count/(invalid_count+valid_count)*100), 2)} %')
    # print(f'valid count: {valid_count} --> {round((valid_count/(invalid_count+valid_count)*100), 2)} %')
    ...
    df_jawac.proba_abd = df_jawac.proba_abd.round(3)
    df_jawac.proba_chest = df_jawac.proba_chest.round(3)

    fig.add_trace(
        go.Scatter(x=df_jawac.index.tolist(), 
                y=df_jawac.data_chest.tolist(), 
                showlegend=False,
                customdata = df_jawac.proba_chest.tolist(),
    ...
    fig.add_trace(
        go.Scatter(x=df_jawac.index.tolist(), 
                y=df_jawac.data_abd.tolist(), 
                showlegend=False,
                customdata = df_jawac.proba_abd.tolist(),
                hovertemplate='<extra><br>proba: %{customdata}</extra>',
                hoverinfo='skip',
                line=dict(
                    color='rgb(57, 119, 175)',
    ...
    legend = 'red(invalid), green(valid)'
...
def parse_opt():
    ...
    parser.add_argument('--model_path', type=str, default='', help='corresponds to the path of the first task model weights')
...
if __name__ == '__main__':
    ...
    # python code/classification/classify_belts.py --show_graph --model 'LSTM' --edf 'code/classification/test_data/belts/patient_data1.edf'
    # python code/classification/classify_belts.py --show_graph --model 'CNN' --edf 'code/classification/test_data/belts/patient_data1.edf'

    # Test cmd lines - patient data from 1 to 13 - MACOS
    # python3 code/classification/classify_belts.py --show_graph --model 'LSTM' --edf 'code/classification/test_data/belts/patient_data1.edf'
    # python3 code/classification/classify_belts.py --show_graph --model 'CNN' --edf 'code/classification/test_data/belts/patient_data1.edf'
