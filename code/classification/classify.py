...
from util import datetime_conversion, f1_m, analysis_cutting, is_valid, block_print, enable_print, hours_conversion, signal_quality, get_value_in_line


def jawac_classification(edf, model, view_graph, plt_save_path):
    """
    Primary function for classifying an analysis made by a patient using the Jawac signal

    Parameters:

    -edf (--edf): edf file path containing time series data (Jawac)
...
def main(p):
    out_dic = jawac_classification(**vars(p))
...
if __name__ == '__main__':
    ...
    # Cmd test lines - patient data from 1 to 13 - LSTM more efficient
