...
def evaluate_model(time_split, time_resampling, epochs):
    ...
    # Save the model
    if not os.path.exists(os.path.dirname(os.path.abspath('runscript.py')) + f'/models/lstm/split_{time_split}_resampling_{time_resampling}'):
        os.mkdir(os.path.dirname(os.path.abspath('runscript.py')) + f'/models/lstm/split_{time_split}_resampling_{time_resampling}')
    if not os.path.exists(os.path.dirname(os.path.abspath('runscript.py')) + f'/models/lstm/split_{time_split}_resampling_{time_resampling}/{epochs}_epochs'):
        os.mkdir(os.path.dirname(os.path.abspath('runscript.py')) + f'/models/lstm/split_{time_split}_resampling_{time_resampling}/{epochs}_epochs')
    filepath = os.path.dirname(os.path.abspath('runscript.py')) + f'/models/lstm/split_{time_split}_resampling_{time_resampling}/{epochs}_epochs/saved_model'
    model_info_file = open(os.path.dirname(os.path.abspath('runscript.py')) + f'/models/lstm/split_{time_split}_resampling_{time_resampling}/{epochs}_epochs/info.txt', 'w')
...
def hyperparameters_tuning(time_split, time_resampling, max_trials, epochs, batch_size):
    ...
    LOG_DIR = os.path.dirname(os.path.abspath('runscript.py')) + f'/models/lstm/ht_tuning/results_split_{time_split}_resampling_{time_resampling}'
    X_train, y_train, X_test, y_test = Preprocessing(time_split=time_split, time_resampling=time_resampling).create_dataset()
    ...
    ht_info_file = open(os.path.dirname(os.path.abspath('runscript.py')) + f'/models/lstm/ht_tuning/results_split_{time_split}_resampling_{time_resampling}/info.txt', 'w')
