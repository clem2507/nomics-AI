...
def load_data(time_split, time_resampling, num_class):
    ...
    if num_class == 2:
        X = np.loadtxt(os.path.dirname(os.path.abspath('util.py')) + f'/data/samples/binomial/split_{time_split}_resampling_{time_resampling}/X.txt')
        y = np.loadtxt(os.path.dirname(os.path.abspath('util.py')) + f'/data/samples/binomial/split_{time_split}_resampling_{time_resampling}/y.txt')
    else:
        X = np.loadtxt(os.path.dirname(os.path.abspath('util.py')) + f'/data/samples/multinomial/split_{time_split}_resampling_{time_resampling}/X.txt')
        y = np.loadtxt(os.path.dirname(os.path.abspath('util.py')) + f'/data/samples/multinomial/split_{time_split}_resampling_{time_resampling}/y.txt')
    ...
    for i in tqdm(range(len(dir_names))):
        ...
        if not dir_names[i].startswith('.'):
            ...
            if os.path.exists(mk3_file) and os.path.exists(edf_file):
                if not os.path.exists(os.path.dirname(os.path.abspath('util.py')) + f'/data/edf_dfs/{dir_names[i]}'):
                    os.mkdir(os.path.dirname(os.path.abspath('util.py')) + f'/data/edf_dfs/{dir_names[i]}')
                else:
                    continue
                ...
                if len(df_mk3) < 1:
                    ...
                    df_mk3.to_pickle(os.path.dirname(os.path.abspath('util.py')) + f'/data/edf_dfs/{dir_names[i]}/{dir_names[i]}_mk3.pkl')
                ...
                df_jawac.to_pickle(os.path.dirname(os.path.abspath('util.py')) + f'/data/edf_dfs/{dir_names[i]}/{dir_names[i]}_jawac.pkl')

    dfs_directory = os.path.dirname(os.path.abspath('util.py')) + '/data/edf_dfs'
...
class Preprocessing:
    ...
    def split_dataframe(self):
        dir_names = sorted(os.listdir(os.path.dirname(os.path.abspath('util.py')) + '/data/edf_dfs'))
        for i in tqdm(range(len(dir_names))):
            if not dir_names[i].startswith('.'):
                df_mk3 = pd.read_pickle(os.path.dirname(os.path.abspath('util.py')) + f'/data/edf_dfs/{dir_names[i]}/{dir_names[i]}_mk3.pkl')
                df_jawac = pd.read_pickle(os.path.dirname(os.path.abspath('util.py')) + f'/data/edf_dfs/{dir_names[i]}/{dir_names[i]}_jawac.pkl')
        ...
        # create directory
        if self.num_class == 2:
            os.mkdir(os.path.dirname(os.path.abspath('util.py')) + f'/data/samples/binomial/split_{self.time_split}_resampling_{self.time_resampling}')
            df_info_file = open(os.path.dirname(os.path.abspath('util.py')) + f'/data/samples/binomial/split_{self.time_split}_resampling_{self.time_resampling}/info.txt', 'w')
        else:
            os.mkdir(os.path.dirname(os.path.abspath('util.py')) + f'/data/samples/multinomial/split_{self.time_split}_resampling_{self.time_resampling}')
            df_info_file = open(os.path.dirname(os.path.abspath('util.py')) + f'/data/samples/multinomial/split_{self.time_split}_resampling_{self.time_resampling}/info.txt', 'w')
    ...
    def create_dataset(self):
        ...
        if self.num_class == 2:
            if dir_path in os.listdir(os.path.dirname(os.path.abspath('util.py')) + '/data/samples/binomial'):
            ...
            if dir_path in os.listdir(os.path.dirname(os.path.abspath('util.py')) + '/data/samples/multinomial'):
                # TODO uncomment for experiments
                # X_train, y_train, X_test, y_test = load_data(time_split=self.time_split, time_resampling=self.time_resampling, num_class=self.num_class)
                self.split_dataframe()
                split_flag = True
        ...
        if split_flag:
            ...
            if self.num_class == 2:
                np.savetxt(os.path.dirname(os.path.abspath('util.py')) + f'/data/samples/binomial/split_{self.time_split}_resampling_{self.time_resampling}/X.txt', X.reshape(X.shape[0], -1))
                np.savetxt(os.path.dirname(os.path.abspath('util.py')) + f'/data/samples/binomial/split_{self.time_split}_resampling_{self.time_resampling}/y.txt', y)
            else:
                np.savetxt(os.path.dirname(os.path.abspath('util.py')) + f'/data/samples/multinomial/split_{self.time_split}_resampling_{self.time_resampling}/X.txt', X.reshape(X.shape[0], -1))
                np.savetxt(os.path.dirname(os.path.abspath('util.py')) + f'/data/samples/multinomial/split_{self.time_split}_resampling_{self.time_resampling}/y.txt', y)
